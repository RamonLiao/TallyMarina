...
（carrying 與 FV 有差時，另出 `DisposalGain` / `DisposalLoss` 平衡差額。Sui gas 之 storage rebate 淨額處理見 §13，Task 6；淨額為負之入帳見 §4.4.1。）

#### §4.4.1 gas 淨額為負（rebate > 成本）之入帳 — MVP

§13.2(b) 之 gas 淨額 = computation + storage − rebate **可為負**（storage rebate 超過 computation+storage，entity 該筆淨收 SUI）。此時 gas 事件非費用支出而為淨流入，不套 §4.4 支出模板，改比照 §4.1 建立新 lot，貸方以 contra-expense + 其他收入兩段處理：

範例：某 tx gas 淨額為負，entity 淨收 0.50 SUI；該 tx accounting_date 之 §6 價格 FV 0.50 USD；當期已認列 `GasFeeExpense` 餘額 0.30 USD。

| Dr/Cr | 科目 | 金額 | 金額來源 | 軌別 |
|---|---|---|---|---|
| Dr | DigitalAssets（SUI，新 lot） | 0.50 | FV（§6 當日價，量 = 淨收量） | 通用 |
| Cr | GasFeeExpense（contra，以當期 gas 費用為限） | 0.30 | 當期 `GasFeeExpense` 餘額上限 | 通用 |
...
**期初導入涵蓋所有 AccountSource（含 CEX 子帳戶 / 託管方）**——CEX 側持倉亦須導入期初 lot；未導入者，後續 §4.6 CUSTODY_MOVE 由該 CEX 子帳戶貸出時，因帳上無足額 lot 將觸發下述 `LOT_SHORTFALL`。
...
| `opening_fv` | FV 軌 entity 必填 | 導入基準日 FV（USD，每 lot）；GAAP ASU 2023-08 FV 軌 entity 之期初 carrying，取導入基準日 §6 價格或 entity 提供之經審計期初 FV。IFRS 成本軌 entity 免填 |
...
**GAAP ASU 2023-08 FV 軌 entity 之期初 carrying 與過渡（§1.2 範圍內資產）**：

FV 軌 entity 之期初 lot carrying **不得只用歷史取得成本**——ASU 2023-08 範圍內資產以公允價值衡量，期初 carrying 應為上線／導入基準日 FV（`opening_fv`）。歷史成本（`unit_cost`）仍保留供揭露（roll-forward cost basis 揭露，§11.2）與處分損益追蹤。歷史成本與導入日 FV 之差額屬**過渡累積影響（cumulative-effect adjustment）**，依 ASU 2023-08 過渡規定調整**期初保留盈餘**，**不進當期損益**（不入 `UnrealizedGain/LossCryptoPnL`）。

過渡分錄（每筆 lot，`opening_fv` > 歷史成本時）：

| Dr/Cr | 科目 | 金額 | 金額來源 | 軌別 |
|---|---|---|---|---|
| Dr | DigitalAssets | opening_fv − 歷史成本 | FV − 歷史成本（per lot） | GAAP |
| Cr | RetainedEarnings | 同額 | RESIDUAL（cumulative-effect） | GAAP |

（`opening_fv` < 歷史成本時借貸反向：Dr `RetainedEarnings` / Cr DigitalAssets。此調整計入期初保留盈餘，屬過渡調整非當期損益。）

**IFRS 成本軌 entity 不受影響**：IAS 38 成本模式期初 carrying 維持歷史取得成本（`unit_cost`），無過渡 FV 調整，導入分錄即上述 Dr DigitalAssets / Cr OpeningBalanceEquity；`opening_fv` 欄位對 IFRS 成本軌 entity 免填。
...
| `GasRebateIncome` | 損益（收入） | §4.4.1 | Sui gas 淨額為負（storage rebate > 成本）時，超出當期 gas 費用之淨流入認列之其他收入 |
| `RetainedEarnings` | 權益 | §7.3 | GAAP ASU 2023-08 過渡：期初導入歷史成本→導入日 FV 之累積影響（cumulative-effect adjustment）計入期初保留盈餘，不進當期 P&L（IFRS 成本軌不適用） |
...
> `RevaluationSurplus` 於 MVP 僅預留於 seed、不產生分錄（revaluation model 列 P1）。其餘八科目 MVP 即用（`RetainedEarnings` 僅 GAAP ASU 2023-08 FV 軌 entity 於期初過渡時使用，見 §7.3）。實際 `account_code` 編碼（科目代號）由 entity 既有 CoA 對接時確定，本清單提供概念名與必要科目集合。
...
- **(b) gas 淨額（可為負）**：`gas = computation + storage − rebate`（storage rebate 為釋放物件退還之押金）；**淨額可為負**（rebate > computation+storage 時）。gas 事件金額須為此淨額，映射 §4.4 FEE_GAS（負淨額入帳見 §4.4.1）。
...
月結為編號 1–9 之順序流程，每步有可判定之 **blocking 條件**，對齊現有 close-readiness lights 概念（六 lights：classification / JE / recon / completeness 為 blocking＝real+derived；pricing / export 之 **light 指示層**於 MVP 為 mock 狀態顯示，production 反轉為 real-and-blocking——**惟缺價本身經 §6.3 `PRICE_MISSING` blocking exception 擋關帳（併入步驟 2、4），mock light ≠ 缺價可放行**）。「一位會計師照章走完一次月結」須能逐步引用本編號（後續 desk check）。
...
| 4 | **期末重估過帳** | 該期所有落 ASU 2023-08 / impairment 範圍資產之期末重估分錄（§5）已產生並 posted；缺價 fail-closed（§6.3）未解 → **擋**：`PRICE_MISSING` 屬 blocking exception（併入步驟 2 之 `exceptions.blocking`），解法為 §6.2 手動 fallback 輸入 PricePoint（標 `LEVEL_2`、記 change log）後補跑重估，不允許帶缺價過帳（缺重估分錄亦使步驟 5 TB / 步驟 6 roll-forward 不成立） |
...
- **blocking = real + derived**（classification / JE / recon / completeness）；pricing / export 之 **light 指示層**於 MVP 為 mock 狀態顯示（不得假綠、不得用 aqua），production 反轉為 real-and-blocking。**缺價不因 pricing light 為 mock 而放行**：`PRICE_MISSING` 屬 blocking exception（§6.3），併入步驟 2 之 `exceptions.blocking` 與步驟 4 重估 gate；解法 = §6.2 手動 fallback 輸入 PricePoint（標 `LEVEL_2`、記 change log）後補跑重估，**不允許帶缺價鎖期**。lock 之 blocking 判定由後端重算、不信 client 輸入。
...
- **CEX 子帳戶對帳範圍聲明**：§14 步驟 3 之自動 recon lights，MVP **僅涵蓋 wallet × coinType**（chain balance 取自鏈上 `addressBalance`，§13.3）。CEX 子帳戶餘額於 MVP **不納入** step 3 自動 lights——其對帳依 §13.4 CSV 對帳單為 statement 側、以人工比對進行；CEX connector API 化列 P1，屆時 CEX 子帳戶納入 step 3 自動 lights 範圍。
...
6. **opening lot 導入**（§7.3）：`lot_movement` 表既有 `OPENING_LOT` 事件路徑（`je_id` 欄位註解明載：「NULL for zero-basis OPENING_LOT (no JE) and pre-feature legacy rows; non-zero OPENING_LOT does get a JE」），落庫路徑已存在；本規格新增之要求為**匯入格式**（§7.3 之 `asset` / `qty` / `unit_cost` / `acquisition_date` / `source_note` 五欄，見 §7.3 表），非新建表——沿現有 `lot_movement` + 對應 `journal_entries` 落地。**`opening_fv_minor`（每 lot）就 GAAP ASU 2023-08 FV 軌 entity 由 deferred 改為 MVP 需要**——承載期初 carrying（導入日 FV）與過渡累積影響調整（§7.3）之落點；IFRS 成本軌 entity 免填。此與第 7 項之逐 lot roll-forward FV 追蹤（仍 deferred）不同：本項僅為期初過渡 carrying，不含歷次期末重估累計。
7. **per-lot 累計 FV 調整追蹤**（§11.2 依賴）：**現有 schema 無此落點**——`lot_movement` 目前只記 `delta_qty_minor` / `delta_cost_minor`（取得/處分之數量與成本異動），不記期末重估（§5）對個別 lot 之累計 FV 調整。為支撐 §11.2 roll-forward 之 realized/unrealized 拆分（處分時將該 lot 先前未實現部分重分類為已實現），需新增落庫維度：逐 lot 記錄歷次期末重估之 FV 調整累計（`lot_id` × 累計調整額，隨每次重估 JE 更新或以新增 movement-like 列追加，沿用 `lot_movement` 之 append-only 精神）。MVP 之 §11.2 roll-forward 先出總額行（不逐 lot 拆分），本落庫需求供未來 realized/unrealized 逐 lot 拆分擴充時使用；不阻 MVP 月結（§14 步驟 6）。落庫最小欄位：`lot_id`（關聯 `lot_movement.lot_id` / PositionLot，§18.1）/ `entity_id` / `period_id` / `opening_fv_minor`（期初 FV，對應 §11.2 roll-forward「期初 FV」列之逐 lot 分解）/ `closing_fv_minor`（期末 FV，對應 §11.2「期末 FV」列之逐 lot 分解）/ `unrealized_adjustment_minor`（當期未實現損益調整，累計值，隨每次期末重估更新）/ `realized_reclass_minor`（處分時自累計未實現調整轉出、重分類為已實現之金額，即 §11.2 realized/unrealized 拆分所需）/ `policy_set_version`（生成當下適用之 PolicySet 版本，§9.2）。

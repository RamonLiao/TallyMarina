...
`acme:pilot-001` 的三個假幣（USDC / WETH / USDT）全部如此，第四個資產 `0x2::sui::SUI` 是真的。後果是那三個**永遠無法登錄** → 永遠 `unregisteredAsset: true` → 依 D12 **永遠擋 close**。

⚠️ **另一個我犯的計數錯誤（留痕）**：brainstorm 階段我對 fixture 檔案跑 `for k, v in d.items()` 統計，數出「5 個資產」，並拿它去**更正** progress.md 寫的「4 資產」。**progress.md 是對的。** 該檔案含**兩個 entity**：`acme:pilot-001`（4 列）與 `opening-lot-recon-test:entity`（1 列，`0xface::tok::TOK`）。我跨 entity 統計，把另一個 entity 的資產算進了 acme。這個「5」被寫進 spec、plan 與七個 task 的 brief，直到 Task 8 的實作者用一句 `COUNT 4` 打掉。

拿「python 印出 5 行」當「這個 entity 有 5 個資產」的證據，中間差了一個 group-by。`0xface::tok::TOK` **不得**被 seed 進 `acme:pilot-001`。
...
| 3c | **manifest 資產揭露區塊**（2026-07-11 whole-branch review A3，實查後 defer） | export bundle 的 manifest 缺 per-coinType 的 `symbol` / `chain_object_id` / `metadata_cap_state`（D16）。**逐列 `origSource` 已在 `journal.csv`，審計師拿得到基本來源可稽核性**；缺的是 manifest 級的 D16 欄位。實作需新 fetch（web 端 `getAssets` client）+ 新 DTO 跨 api/web —— 與 3b（close-window）、§10-2（materiality 主檔化）同屬 H1 依賴的控制層。**且 `metadata_cap_state` 的下游語意本身未定**（§3.3：`decimals` 是否在 `MetadataCap` 可更新欄位之列，未查證）——先補揭露層等於先蓋屋頂再確認地基。一起在 H1 輪做。 |

...
- v1.2（2026-07-13，dual-review 外部輪）：identity① 控制語意由「fold 恆等式（closingFV === openingFV + additions − disposals + gains − losses）」改為 **per-coin GL tie**（該 coin 的 lot-side closingFV === 該 coin 的 DigitalAssets GL closing）。理由：外部 review 坐實 fold 版對任何資料代數恆真（純為 lot-side 數字的重述），永遠 PASS，違反 Rule 9（不能 fail 的檢查是錯的）；per-coin GL tie 是真跨 store 對帳（lot_movement+lot_valuation vs journal_entries），單一 coin 帳被挪走時只有該 row 紅。§11.2 六欄算術保留為 ASU roll-forward **呈現層**，非控制。identity②（tbTie）降為 per-coin tie 的彙總推論。附帶：LOCKED drift 擴充 completeness 維度（凍結 light 對現算 identitiesOk），legacy NULL/malformed period_id 於 TB/RF 改 fail-closed 可見（balanced=false / tbTie.ok=false，非 500）。

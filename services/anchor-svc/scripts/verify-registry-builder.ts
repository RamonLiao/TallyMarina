 ...
 *
 * Usage:
 *   SUI_PK=suiprivkey1... npx tsx scripts/verify-registry-builder.ts
 *
 * The script uses the keypair to extract the owner address, then queries the live
 * testnet for the AnchorCap owned by that address, resolves the entityId, and
 * verifies the chain/cap object ids match.
 */
...
async function main() {
  ...
  if (!pk) {
    throw new Error('SUI_PK env var not set (e.g. from the e2e bootstrap keypair)');
  }

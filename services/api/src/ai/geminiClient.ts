...
export interface GeminiClient {
  generateJson<T>(model: string, prompt: string, schema: GeminiSchema): Promise<T>;
}

export function makeGeminiClient(apiKey: string): GeminiClient {
  return {
    generateJson<T>(model: string, prompt: string, schema: GeminiSchema): Promise<T> {
      return callGeminiWithKey<T>(apiKey, model, prompt, schema);
    },
  };
}

async function callGeminiWithKey<T>(
  apiKey: string,
  model: string,
  prompt: string,

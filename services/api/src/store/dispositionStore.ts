...
export function upsertDisposition(db: Db, row: DispositionRow): void {
  db.prepare(
    `INSERT INTO exception_disposition (category, event_id, entity_id, state, reason_code, reason_note, decided_by, decided_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  ...
  ).run(row.category, row.eventId, row.entityId, row.state, row.reasonCode, row.reasonNote, row.decidedBy, row.decidedAt);
}

export function appendDispositionLog(db: Db, row: DispositionRow): void {
  db.prepare(
    `INSERT INTO exception_disposition_log (category, event_id, entity_id, state, reason_code, reason_note, decided_by, decided_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
  ).run(row.category, row.eventId, row.entityId, row.state, row.reasonCode, row.reasonNote, row.decidedBy, row.decidedAt);
}

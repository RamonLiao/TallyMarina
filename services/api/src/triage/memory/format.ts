...
export const MEMORY_HIT_TEXT_CAP = 300;

/**
 * Escape/truncate untrusted text before interpolating into a rendered record or few-shot
 * bullet (spec R2): collapse newlines so a note/hit can't forge a fake `- bullet` line, and
 * cap length so a single hostile record can't blow up prompt/audit-record size.
 */
export function sanitizeForPrompt(text: string, cap: number): string {
...
export function renderMemoryRecord(r: MemoryRecord): string {
  ...
  return r.note ? `${body} — human note: ${sanitizeForPrompt(r.note, NOTE_TEXT_CAP)}` : body;
}

export function renderFewShotBlock(hits: MemoryHit[]): string {
  if (hits.length === 0) return '';
  const lines = hits.map((h) => `- ${sanitizeForPrompt(h.text, MEMORY_HIT_TEXT_CAP)}`).join('\n');
  return [

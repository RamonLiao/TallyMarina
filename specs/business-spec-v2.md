# Sui Agentic Subledger 商業／產品規格書 v2.0
...
### 2.5 Demo MVP 與 paid-pilot MVP

| 範圍 | Demo MVP | Paid-pilot MVP |
|---|---|---|
| 目的 | 展示端到端與一個 fail-closed exception | 完成一個真實期間 parallel close |
| Entity/貨幣 | 單 entity、單功能性貨幣 | 單 entity、單功能性貨幣 |
| Source | Sui wallet；可用重現 fixture | Sui wallet |
| Event | receipt、payment、internal transfer、swap、gas | 同左，僅 5 類 |
| 準則 | 可展示兩模板差異，不宣稱 compliant switch | IFRS 或 US GAAP 二選一，由客戶核准 |
| Cost basis | FIFO | FIFO |
| Approval | 人工 | 人工；AI 不可核准 |
| Export/recon | Generic CSV、wallet quantity recon | Generic CSV、wallet quantity recon、manual ERP acknowledgement |
| Walrus | optional demo publication | 不在首個 paid pilot 必要範圍 |

IFRS 重估、staking、CEX、WAC、Walrus 完整包、多實體與完整 disclosure 均退出首個 paid pilot。
...
唯一權威 schema 為《會計規格書 v2》第 3 章 Canonical Event Registry 及 leg taxonomy；本文件不得另造 code。Paid-pilot 僅啟用：

- `DIGITAL_ASSET_RECEIPT`
- `DIGITAL_ASSET_PAYMENT`
- `INTERNAL_TRANSFER`
- `SPOT_TRADE_SWAP`
- `GAS_FEE`

CEX、staking、bridge/wrap、LP、rebase、fork 與期末衡量保留於 registry，但不屬首個 paid pilot。

#### 會計與控制

- 單一準則 PolicySet（IFRS 或 US GAAP 二選一）；具體判斷由客戶具名核准。
- FIFO cost basis。
- Event → JE mapping rule。
- classification/policy/JE approval 分離；JE/export 使用 `APPROVED → EXPORTED → ACKNOWLEDGED → POSTED → REVERSED`，拒絕為 `REJECTED`。
- wallet-to-subledger reconciliation。
- Generic ERP-ready CSV。
- Audit log、policy version、rule version、price lineage。
- 內部 immutable close manifest；Walrus 只為 optional async publication。
...
- 單 entity、單功能性貨幣、Sui wallets。
- Canonical registry 中 5 個 paid-pilot events。
- FIFO lots 與完整 pricing exception。
- Policy／mapping UI。
...
唯一 canonical schema 為《會計規格書 v2》第 5.1 節；本文件不另造欄位。`accounting_standard` 設於 book；asset/position 層保存 scope test、`measurement_model` 與 out-of-scope route。`stablecoin_treatment`、`crypto_classification_default`、`impairment_or_fair_value_mode` 等單欄位預設不得驅動 auto-post。
...
|---|---|---|---|
| Paid Pilot | 固定一期間、1 entity、5 events、CSV | （推測/TODO）導入費 + pilot fee | 導入人日、會計 review、data/AI、support |

...
企業使用穩定幣、鏈上資產、交易所與 DeFi 協議的情境正在增加，典型用途包括跨境收付款、資金調度、交易、做市、質押與協議營運。2025 年穩定幣調整後年交易量達數兆美元級別，且 Stripe（收購 Bridge）、Visa、JPMorgan 等持續擴張企業穩定幣產品，顯示企業採用正在成長。[來源: https://a16zcrypto.com/posts/article/state-of-crypto-report-2025/] 傳統 ERP、總帳與 Treasury Management System 通常無法原生理解錢包、鏈上交易、物件狀態與協議事件；主流 vendor 定位（如 Cryptio、Bitwave）是在鏈上資料與 ERP 之間建立專門的轉譯層（digital asset subledger），本產品沿用此邊界。（此為主流 vendor 定位,非已驗證之業界普遍實踐）
...
Cryptio、Bitwave 等主流 vendor 將自身定位為 ERP 前的 digital asset subledger，ERP 仍為正式總帳與財務報導 system of record；這是 vendor positioning，不宣稱為全產業一致做法。本產品採此邊界，以降低導入阻力及控制範圍。
...
穩定幣在企業 treasury、跨境 B2B 與供應商結算的採用持續擴張；a16z 2025 報告彙整約 46 兆美元年度穩定幣交易量及企業支付基礎設施擴張。[來源: https://a16zcrypto.com/posts/article/state-of-crypto-report-2025/] 是否轉化為本產品可服務需求仍是中長期市場假設。
...
### 2.5 唯一 capability matrix

本表是 Demo、Paid Pilot 與 Post-pilot 範圍的唯一權威來源。§5、§9、§17 及其他章節只能引用本表，不得重述或擴張範圍。

| Capability | Demo | Paid Pilot | Post-pilot |
|---|---|---|---|
| 目的 | 端到端展示與 fail-closed exception | 一個真實期間 parallel close | 依證據 gate 擴張 production 能力 |
| Entity／貨幣 | 單 entity、單功能性貨幣 | 單 entity、單功能性貨幣 | 多實體、多幣別 |
| Source | Sui wallet；可用重現 fixture | 僅指定 Sui wallet | CEX、custody、DeepBook、受控 CSV、其他鏈 |
| Event | 5 個 paid-pilot event，可用 fixture | 僅 `DIGITAL_ASSET_RECEIPT`、`DIGITAL_ASSET_PAYMENT`、`INTERNAL_TRANSFER`、`SPOT_TRADE_SWAP`、`GAS_FEE` | registry 其餘事件逐項解除 gate |
| 準則／衡量 | 可展示模板差異，不宣稱 compliant switch | 僅 IFRS；SUI 為 IAS 38 成本模式；USDC 經四層 StablecoinAssessment，結論前為 `PENDING_ACCOUNTING_REVIEW` | US GAAP、IFRS 重估及其他模型另行核准 |
| Cost basis | FIFO | FIFO | WAC／specific identification 等經核准方法 |
| Approval | 人工 | 人工；AI 不可核准政策或 JE | 仍維持 maker-checker |
| Export／recon | Generic CSV、wallet quantity recon | Generic CSV、wallet quantity recon、manual ERP acknowledgement | 原生 ERP connector、雙向 recon |
| Audit snapshot | 內部 immutable snapshot；Walrus 可展示 | 內部 immutable audit snapshot 必須；Walrus publication optional | Walrus／其他 evidence publication 依治理核准 |
| 明確排除 | production claim | CEX/custody、DeepBook、staking、bridge/wrap、LP、fork/rebase、期末重估、broker-trader、cash-flow auto classification、多實體、完整報表 | 依 roadmap 與 Accounting Decision Record 解鎖 |
...
- 依 §2.5 的階段範圍新增 AccountSource；CEX、custody、受控 CSV 為 Post-pilot。
...
IFRS 下，IFRIC 2019 對其事實範圍內的 cryptocurrency holdings 指向 IAS 2（正常營業過程出售；broker-trader 特例）或 IAS 38。[來源: https://www.ifrs.org/content/dam/ifrs/supporting-implementation/agenda-decisions/2019/holdings-of-cryptocurrencies-june-2019.pdf] US GAAP ASU 2023-08 僅適用符合六項 scope 條件的特定 crypto assets。[來源: https://dart.deloitte.com/USDART/home/codification/assets/asc350-60/roadmap-digital-assets/chapter-2-classification/2-3-crypto-assets-within-scope] 穩定幣須依 instrument、issuer、jurisdiction 與條款版本判斷；IFRS 無明文將 stablecoin 一律分類為現金等價物，實務存在分歧。
...
- 鎖定 period，阻止一般使用者修改。
- 產生必要的內部 immutable close package；若 tenant 啟用選配 publication，再非同步產生 Walrus reference。
...
- 依統一狀態機管理 `APPROVED → EXPORTED → ACKNOWLEDGED → POSTED`，例外為 `REJECTED`，更正為 `REVERSED`。
- 保留匯出檔 hash、mapping version 與操作人。

MVP 不承諾 ERP API 雙向同步。CSV 匯入後，使用者須上傳 import report 或輸入 ERP batch/JE ID、匯入人與時間完成 manual acknowledgement；未 acknowledgement 不得標記 posted。
...
- 驗證內部 immutable snapshot hash；若已選配發布，可另取得並驗證 Walrus blob reference。
...
首個 Paid Pilot 的 source、event、準則、資產、cost basis、export、reconciliation 與 audit snapshot 範圍，以 §2.5 唯一 capability matrix 為準。Canonical event code 與 leg taxonomy 以《會計規格書 v3》第 3 章為準；PolicySet 與 Accounting Decision Record gate 以該文件第 5 章及附錄 C 為準。

JE／export 不使用線性狀態簡寫。唯一 transition table 見《會計規格書 v3》§6.7；`REJECTED` 可依 guard 從 `APPROVED`、`EXPORTED`、`ACKNOWLEDGED` 或 line-level partial acceptance 進入，絕非 `POSTED → REJECTED`。已 `POSTED` 的更正只能 `REVERSED` 後建立 replacement JE。
...
- Paid Pilot 已支援 manual ERP acknowledgement；原生 ERP acknowledgement 與雙向 reconciliation 為 Post-pilot。
...
3. 對 package 計算 hash。
4. 鎖定內部 immutable snapshot；選配時才將最小 manifest/hash 或核准的加密封裝上傳 Walrus。
5. 內部 `AuditSnapshot` 成功即完成 accounting close。
6. Walrus publication 以非同步 queue 執行，UI 顯示 reference、hash、建立人與驗證狀態。

Walrus 的持久性、成本、保留期、法律證據效力與企業資料治理能力列入 Validation Backlog。敏感資料不得以明文公開；只允許最小 manifest/hash 或經核准的加密封裝。
...
| Walrus Snapshot | Optional demo | 非同步 evidence publication，不得阻擋 close |
...
| GraphQL／Indexer | 歷史查詢、期間篩選、報表與重跑 | 驗證後採用 | GA/穩定狀態存疑；需保留 RPC/自建 indexer fallback |
...
| DeepBook | 交易、fee 與 position 事件來源 | Post-pilot；範圍見 §2.5 | 建立 Sui DeFi accounting 差異化 |
| Walrus | 選配 evidence publication | Demo 可展示；Paid Pilot 非必要；範圍見 §2.5 | 不得成為 close dependency |
...
Demo 能力邊界與選配項目只依 §2.5；本節不另列 source、event 或 Walrus 步驟。
...
- 內部 immutable manifest 建立失敗不得 close；Walrus publication 失敗只進非同步重試。
...
- 敏感財務、個資與憑證不得以明文寫入公開鏈或公開儲存；此為 fail-closed 安全控制。
- 支援 tenant isolation。
- 高風險操作與大批量匯出需通知。
...
能力邊界直接引用 §2.5 的 Demo 欄；本章只描述交付順序，不另列 source、event 數、準則或 Walrus 驗收範圍。

**完成定義**：從真實或可重現測試交易走完端到端流程，且每筆 JE 可追溯來源與規則。

### Phase 1：Paid Design Partner Pilot

能力邊界直接引用 §2.5 的 Paid Pilot 欄。交付重點為完成對應 Accounting Decision Records、golden fixtures、Policy／mapping UI、close checklist、maker-checker 與 period lock。3–5 個 design partners 是 GTM 假設，不是既有承諾。（推測）
...
Phase 3–4 不屬目前 roadmap，移至附錄 C Vision；未達 5 個付費 production 客戶及對應 evidence gate 前不配置承諾資源。
...
下表整合 R1 市場查證；「Sui 支援」只表示公開確認，並不代表 DeepBook、staking、object-level accounting 或 production quality。價格與實際能力仍須 benchmark。

| 產品／替代方案 | 公開定位 | ERP／企業整合 | Sui 支援 | 本產品待驗證差異 |
|---|---|---|---|---|
| Cryptoworth | Crypto subledger、成本基礎、分錄 | NetSuite 等 | 未證實 | DeepBook/object coverage、policy trace、close 工時 |
| TRES Finance | Web3 financial data、subledger、reconciliation | 多 ERP | 明確支援 | 同上；不得以「率先支援 Sui」主張 |
| Bitwave | Enterprise digital asset accounting/tax | 多 ERP | 未證實 | 同上 |
| Cryptio | Institutional digital asset accounting | NetSuite／ERP API | 明確支援；staking coming soon | DeepBook/object 深度與導入工時 |
| TaxBit | Enterprise accounting/tax/compliance | 企業整合 | 未證實 | 窄 paid-pilot 成果 |
| Integral | Crypto accounting | 會計輸出待實測 | 明確支援 [來源: https://integral.xyz/integrations/sui] | DeepBook/object depth、policy lineage |
| ZenLedger | 稅務、成本基礎與報告 | 企業 ERP 深度待實測 | 未證實；撤回原「已支援」說法 | 子帳 close workflow |
| Excel＋外包會計師 | 高彈性、依賴人工 | CSV／手動 JE | 可人工處理 | 重跑、lineage、例外工時 |
| 自建 indexer＋腳本 | 高客製與技術深度 | 自建 | 可深度支援 | 維護總成本與會計控制 |

### 10.2 競爭判斷

- 市場類別已被驗證，但「有市場」不等於本產品自然具有競爭優勢。[需查證]
- 多數成熟競品可能在 chain coverage、ERP connectors、企業安全與客戶信任上領先。（推測）
- 已有 TRES、Cryptio、Integral 三家公開支援 Sui；「Sui-native」本身不是 moat。本產品僅以 DeepBook/object 解析深度、policy trace 與 close 工時改善作為待 benchmark 的差異假設。
...
- ERP-ready export。
- Walrus audit manifest 為 optional，不是 Pilot 驗收條件。
...
| 實體 | 目的 | 關鍵欄位摘要 |
|---|---|---|
| Entity | 法人／帳務主體 | timezone、fiscal calendar；準則不放 entity |
| Book | 帳簿 | accounting_standard、functional/reporting currency、period |
...
| AssetProfile | 跨帳簿技術主檔 | identifier、decimals、technical type；不得保存唯一 accounting class |
| AssetPositionAssessment | 帳簿/部位會計判斷 | scope test、measurement model、out-of-scope route、evidence/version |
| StablecoinAssessment | 四層穩定幣判斷 | instrument、issuer、jurisdiction、terms version、expiry/triggers |
| PricePoint | 價格／FX | timestamp、source、method、quality |
| PositionLot | 成本批次 | acquired／remaining qty、unit cost、method |
| PolicySet | 帳簿會計政策引用 | canonical schema、子政策 ids、version |
...
- Entity 1:N Book；Book 1:N AssetPositionAssessment，同一 Book 可並存多個 measurement models。
- AccountSource 1:N RawTransaction。
- RawTransaction 1:N NormalizedEvent。
- NormalizedEvent N:1 AssetProfile，並依 book/position 關聯 AssetPositionAssessment、PricePoint、PositionLot。
...
Paid Pilot 僅以 §2.5 的 Paid Pilot 欄為範圍，且只有在以下條件同時成立時才視為完成：
...
- 可生成並鎖定含來源、政策、規則、價格、FX、核准與 export hash 的內部 immutable audit snapshot。
- Walrus publication 為 optional asynchronous publication；未發布或發布失敗不得阻擋 close、不得使 Paid Pilot 驗收失敗。
...
### A.1 PolicySet canonical schema

唯一 canonical schema 為《會計規格書 v3》第 5.1 節；本文件不另造欄位。active create/update schema 不含 `stablecoin_treatment`、`default_token_classification` 或 `crypto_classification_default`；它們只可由 migration adapter 讀取，轉換後不得持久化為 active decision source。
...
IFRIC 2019 agenda decision 對其事實範圍內的 cryptocurrency holdings 指向 IAS 2（正常營業過程出售；broker-trader 適用 IAS 2.3(b)）或 IAS 38；ASU 2023-08 則有獨立六項 scope criteria。[來源: IFRIC, Holdings of Cryptocurrencies, June 2019；Deloitte Roadmap ASC 350-60 §2.3] 實際適用仍須由具名專業會計人員依客戶事實核准。

---

## 附錄 C：Vision（不在首個付費 pilot）

- Stablecoin AP/AR、TMS、銀行/payment rails 與 cash forecasting。
- 受控 agentic close planning、evidence preparation 與非資金型 workflow。
- 任何資金執行、正式 ERP posting、多實體 consolidation 均需另立產品與責任規格。

## 附錄 D：Open Decisions Register

| Decision | Owner | Deadline | Decision criteria |
|---|---|---|---|
| Pricing/FX provider | Product + Accounting Lead | paid pilot 前 6 週 | 30 日 Sui coverage、歷史深度、授權、SLA、每客戶 COGS、fallback |
| 首個 ERP | Product + Pilot Controller | pilot 簽約後 2 週 | 客戶實際系統、CSV import、manual acknowledgement、防重複與 reversal |
| 政策審查機制 | Accounting Lead | 第一個 PolicySet 啟用前 | 具名 reviewer、citation、golden fixtures、SoD、版本生效及責任界線 |
| Walrus 策略 | Security + Legal | production go/no-go 前 | hash/加密包、retention/deletion/key recovery、審計師接受度、法律效力 |

## 附錄 E：Validation Backlog

### E.1 TAM/SAM/SOM bottom-up framework

不得填入未有來源的市場數字。計算框架：

- TAM = 各 segment 可識別法人帳戶數（TODO）× 可實現 ACV（推測/TODO）。
- SAM = TAM 中符合 Sui/交易量/ERP/財務人員/資料區域條件的帳戶數（TODO）× ACV。
- SOM = 24 個月可觸達 named accounts（TODO）× evidence-based conversion × ACV。
- 每個輸入保存來源、年份、owner、confidence 與更新日。

### E.2 Evidence-gated GTM milestones

| Milestone | Evidence gate | 狀態 |
|---|---|---|
| Named accounts | 30–50 個具名、符合 ICP 的帳戶及聯絡路徑 | TODO；未達成 |
| 訪談 | 至少 15 次訪談紀錄、close baseline、替代方案與預算 | TODO；未達成 |
| Data samples/LOI | 5 份匿名資料樣本與具體 LOI | TODO；未達成 |
| Paid pilots | 3 個有費用、範圍與 acceptance 的 parallel close | TODO；未達成 |
| Production | 先達 5 個付費 production 客戶再擴下一 segment | TODO；未達成 |

### E.3 三包定價框架與 COGS

| Package | Scope | 價格假設 | COGS 欄位 |
|---|---|---|---|
| Paid Pilot | 範圍依 §2.5 | （推測/TODO）導入費 + pilot fee | 導入人日、會計 review、data/AI、support |
| Growth | 已驗證 sources/rules/recon | （推測/TODO）年約 + transaction tier | provider、cloud、support、維護 |
| Enterprise | SSO/ERP/SLA/區域要求 | （推測/TODO）annual minimum + implementation | security、ERP、CS、legal/compliance |

所有實數需以至少 3 個客戶正式報價驗證；未驗證前不得作為 traction 或 unit-economics 事實。另需追蹤 implementation hours、data/AI/storage、會計顧問、support、gross margin 與 payback。

### E.4 Pilot 前驗證

- 同一組 Sui fixtures 對至少 3 個競品/自建流程實測 parser coverage、錯誤率、DeepBook/object 深度、close 工時、導入時間與成本。
- Pricing/FX provider 的 30 日 coverage、授權、歷史深度、principal-market 支援與缺價人工工時。
- Walrus 的法律證據效力、retention/deletion、key recovery 及客戶/審計師接受度。
- SOC 2、資料駐留、DPA、subprocessor 與 retention 要求，以 3 個目標客戶 questionnaire 驗證。
